...
const WriteThanks = () => {
  ...
  useEffect(() => {
    //sender name 조회
    if (userData) {
      UserService.read(userData.user_id).then((res) => setSenderName(res.username));
    }
  }, [messageCard, dispatch]);

  //메세지 글자수 세기
  const countCardText = (event: React.KeyboardEvent<HTMLTextAreaElement>): void => {
  ...
  //카테고리 활성화하기
  const activeCategory = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
  ...
  return (
    <>
      <Layout>
        ...
        <Header />
        <img src={writeCardRabbit} alt='writeCardRabbit' css={styles.writeCardImg} />
        <BubbleMessage style={{ fontSize: '16px' }}>
          {state.receiverName}에게 남길
          <br />
          감사 카테고리를 선택해보자!&nbsp;(깡총)
        </BubbleMessage>
        <p css={styles.guideText} onClick={() => setGuideClicked(!guideClicked)}>
          이게 뭐하는 건데?
        </p>
        {guideClicked && <GuideModal guideClicked={guideClicked} setGuideClicked={setGuideClicked} />}
        ...
        <div css={styles.writeCardWrapper}>
          <div css={styles.writeCardBox}>
            <p>
              To. <span>{state.receiverName ? state.receiverName : '나'}에게</span>
            </p>
            <textarea
...
export const CATEGORIES = ['감사', '사랑', '도움', '응원', '위로', '존경', '존재', '그냥'];
export const CATEGORIES_ENG = ['thanks', 'love', 'help', 'comfort', 'cheering', 'respect', 'existence', 'just'];
